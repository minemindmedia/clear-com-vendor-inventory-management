...
<style>
    ...
    .wcvm-orders.po-back-order {
        border: 1px solid #f8961e;
        box-shadow: 0 0 10px #f8961e;
    }
...
//    $status = isset($_REQUEST['status']) ? $_REQUEST['status'] : 'new-order';
    ...
    if ($orders) {
        ...
        ?>
                <div class="flex">
            <div class="flex-1">
                <?php $table_headers = $table->get_back_order_columns_list(); 
...
//        }
        if (!in_array($order->order_id, $printed_po_numbers)) {
            ...
            $records = true;
            ?>
            <form style="clear: both" class="purchase-order border-2 border-t-8 border-pink-600 p-8 mb-4 bg-gray-50"  id="<?= esc_attr($order->order_id) ?>" action="<?= site_url('/wp-admin/admin.php?page=wcvm-epo') ?>" method="post">
...
//                                $order_product_Qty = $order->product_ordered_quantity;
                                $order_product_Qty = $order->product_quantity_back_order;
...
<?php $table_headers = $table->get_on_order_columns_list(); 
            require_once plugin_dir_path(__FILE__) . 'po-status-bar.php';
        ?>
        <div class="flex border-2 border-t-8 border-pink-600 p-8 mb-4 bg-gray-50 text-lg text-semibold">
            There is nothing currently on back order.
        </div>
